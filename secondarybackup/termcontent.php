	...
	<center>
		...
		<?
			...
			<?php
					...
					else if ($term["type"] == "team")
					{
						$query = "select id from Team where name='" . $term["name"] . "';";
						$termId = executeQuery($db, $query);
						$termId = $termId[0]["id"];
						$query = "select * from Game where homeTeamId=" . $termId . " or awayTeamId="
							. $termId . " order by startTime desc;";
						$games = executeQuery($db, $query);
						if (count($games) > 0)
						{
							$game = $games[0];
			?>
						
							<a href="./game.php?id=<?= $game["id"] ?>">Go To Game</a>
			<?php
						}
						else
						{
